...
from aiogram_toolkit.sqla_dashboard.utils.schema import get_schema_by_model_name, make_schema_nullable
from aiogram_toolkit.sqla_dashboard.utils.validation_utils import dict_resolve_complex_objs
from sqla_dashboard.memory.memory_cluster import MemoryCluster
...
async def get_current_instance_(
    ...
    """
    Retrieve one or more properties from the current instance in the memory stack.

    Args:
        memory (MemoryCluster): The memory context.
    ...
    """
    stack_list = await memory.detail.stack.stack_list.get()
    if not stack_list:
        return None

    current = stack_list[-1]
    class_name, i_id = current['class_name'], current['id']
    instance = memory.detail.stack.instance(class_name, i_id)
...
async def update_current_instance_(
    ...
    """
    Update a specific field of the current instance in the memory stack.
